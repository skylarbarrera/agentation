...
const pluginReanimatedRoot = path.resolve(monorepoRoot, 'plugins/plugin-reanimated');

const config = getDefaultConfig(projectRoot);

// Watch library, monorepo, and external packages for hot reload
config.watchFolders = [libraryRoot, monorepoRoot, reanimatedPauseStateRoot, pluginReanimatedRoot];
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  // Resolve @agentation/plugin-reanimated from workspace
  if (moduleName === '@agentation/plugin-reanimated') {
    return {
      filePath: path.resolve(pluginReanimatedRoot, 'dist/index.js'),
      type: 'sourceFile',
    };
  }
