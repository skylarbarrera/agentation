 ...
 *
 * V2 Protocol Support:
 * - Sessions for grouping annotations by route
 * - Intent/severity classification
 * - Status lifecycle (pending -> acknowledged -> resolved)
 * - Thread messages for agent/human replies
...
export interface Annotation {
  ...
  // ==========================================================================
  // V2 Protocol Fields (MCP/Sync Support)
  // ==========================================================================

  /**
   * Session this annotation belongs to
   * Used for grouping annotations in MCP server
   */
  sessionId?: string;

  /**
   * URL (web) or route name (RN) where annotation was created
   * Kept for protocol parity with web
   */
  url?: string;

  /**
   * User's intent classification
   * What action they want taken
   */
  intent?: import('./v2').AnnotationIntent;

  /**
   * Priority/severity level
   * How important this feedback is
   */
  severity?: import('./v2').AnnotationSeverity;

  /**
   * Workflow status
   * Tracks annotation through review lifecycle
   */
  status?: import('./v2').AnnotationStatus;

  /**
   * Conversation thread
   * Replies from human and agent
   */
  thread?: import('./v2').ThreadMessage[];

  /**
   * ISO 8601 creation timestamp
   * More precise than Unix timestamp for sync
   */
  createdAt?: string;
  ...
  /**
   * Local sync tracking
   * Last session ID this annotation was synced to
   * @internal Used by sync infrastructure
   */
  _syncedTo?: string;

  // ==========================================================================
  // Web Parity Fields (kept for protocol compatibility)
  // ==========================================================================

  /**
   * CSS classes on the element
   * Web-only, not populated in RN
   */
  cssClasses?: string;
  ...
  /**
   * React component names from devtools
   * Web-only format, RN uses componentType
   */
  reactComponents?: string;

  /**
   * Multiple element bounding boxes
   * Web-only, for multi-select
   */
...
export interface AgenationProps {
  ...
  /**
   * MCP HTTP server URL
   * Web parity: uses 'endpoint' prop name
   * Example: 'http://192.168.1.100:4848'
   */
  endpoint?: string;
   ...
   * Useful for persisting session ID
   */
  onSessionCreated?: (sessionId: string) => void;

  /**
   * Webhook URL for annotation events
   * Events: annotation.created, annotation.updated, etc.
   */
  webhookUrl?: string;
